...
For the smallest clusters we have produced, a plausible model is provided by calculated minimum energy structures of Ar$_N$Xe$_{38-N}$ (Fig.\ 7 in Ref.\ \citenum{marques}).
...
If we produce a spectrum only from those electrons recorded as part of a two-electron coincidence (colored symbols in Figure \ref{figure:ival}), the apparent intensity drops.
...
Clearly, spectra for the three different cluster sizes shown in Figure \ref{figure:icd_12} show significant differences: Spectra for the larger cluster, in comparison acquire more intensity in the 1-1.5 eV region and have less intensity for energies below 0.5 eV.
...
\end{figure}
%
We now discuss which fraction of Ar 3s vacancy states relax via ICD or ETMD.
A method to derive this figure has been established, which we apply here.\cite{foerstel_2013}
...
The result gives the branching ratio of decay via ICD/ETMD vs. all channels, including those not involving electron emission.
...
This virtually agrees with the fit to the total spectrum in Figure \ref{figure:ival}a, but not in \ref{figure:ival}b, which corresponds to ICD/ETMD efficiencies of one and significantly smaller than one.
...
For the gas mixture with a Xe$_{\rm in}$ of 1.2\,\%, we therefore show results from fits using two different models appearing both justifiable.
