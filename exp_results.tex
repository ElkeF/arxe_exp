...
We found an Ar 3s autoionization efficiency which, for clusters with the lowest Xe content (Xe$_{\rm cl}$ approx. 10-12\,\%), is smaller than unity, and further decreases with decreasing cluster size. Two main reasons can be identified that cause this decrease in autoionization efficiency. First, is the distance between the initially ionized Ar atom to the decay partner too large, then the excited Ar ion can decay via fluorescence before it can autoionize. And second, if there is no decay partner available at a sufficiently large distance, then a decay is energetically not possible. In both cases, a small Xe content would lead to a decrease in autoionization efficiency. The first case should not occur in small clusters while the second should strongly increase with decreasing cluster size. For larger Xe$_{\rm cl}$, the values are compatible with unity, within the accuracy of our experiment. 
